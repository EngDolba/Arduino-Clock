...
{
  if(alarmHour==0 && alarmMinutes==0) return;
  MotorClock::setCurrentTime();
...
double MotorClock::setStopwatch() {
    ...
    while(true)
    {
      ...
      if(readState==HIGH)
      {
        unsigned long int passedTime = curr-millis();
        int passedTimeInMinutes = passedTime/6000;
        Stepper stepper(2038,stepper1,stepper3,stepper2,stepper4);
...
{
  MotorClock:setColor(255,137,255);
...

{
  while(true)
  {
    ...
    if(state==HIGH)
    {
      ...
      return MotorClock::setStopwatch();
    }
  }
}
}
void MotorClock::MotorClock:setColor(int redValue, int greenValue, int blueValue) {
